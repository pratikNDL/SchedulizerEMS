...
function TableRow({data, id, cols, index, deleteHandler , clickHandler= async () => {} }: TableRowType) {
  ...
  return (
    <div className="flex flex-col justify-center items-center" onMouseEnter={() => {setShow(true)}} onMouseLeave={() => {setShow(false)}}>
        <div key={id} onClick={() => clickHandler(id)}  className={`grid w-full border-t-2 border-gray-400  ${index%2 ? 'bg-blue-100': ''} ${show ? 'border-b-2 bg-green-200 ': ''} `} style={{gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))`}}>    
        ...
        { deleteHandler ?
        <div className={`${show? "": 'h-0 opacity-0'} flex  justify-center overflow-hidden transition-all duration-300 border-r-2 border-gray-400 w-full`}>

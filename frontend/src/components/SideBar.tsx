...
function SideBar() {
  ...
  return (
    <div className="h-full w-fit p-2 flex flex-col gap-2 absolute z-10 bg-white border-gray-500 border-r-2 shadow-2xl" >
  ...
  return (
    <Link to={path}>
        <div className='flex items-center p-1 cursor-pointer '>
            <div className={`bg-blue-300  p-2 flex justify-center items-center rounded-full hover:shadow-xl hover:bg-gray-300 `}>
                <img src={icon} alt="" width={25} className='fill-red-200'/>
            </div>
            <div className={`${isExpanded ? 'ml-3': 'ml-0 w-0 h-0 -translate-x-5 opacity-0 '} overflow-hidden font-semibold text-gray-600  transition-all duration-100 `}>
                {title}
            </div>
        </div>
    </Link>
